...
const Blog=()=>{
    ...
    //búsqueda que pone el usuario en el input de searchBar
    const [busqueda, setBusqueda]=useState("");
    //resultado de la búsqueda del usuario en SearchBar
    const [resultados, setResultados]=useState([])
    var resultadoInput=[];
    
    //mostrar todos los datos de los post
    const mostrarDatos=()=>{
    ...
    //Evento onChange que detecta lo escrito en el input de SearchBar
    const handlerChange=e=>{
        setBusqueda(e.target.value);
        console.log(busqueda)
        filtrar(busqueda)
    }
    //Se buscan coincidencias entre lo escrito por el usuario y los post
    const filtrar=(palabraUsuario)=>{
        for(let i of data){
            if(i.title.toString().toLocaleLowerCase().includes(palabraUsuario.toLowerCase())){
                resultadoInput.push(i)
            }
        }
        setResultados(resultadoInput)
    ...
    return(
        <div className="blog">
            ...
            <form className="flex items-center">   
                <label htmlFor="simple-search" className="sr-only">Busca tu post favorito</label>
                    <div className="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Busca por título tu post favorito" 
                            required
                            onChange={handlerChange}/>
                    </div>
                    <button 
                        type="submit" 
                        className="p-2.5 ml-2 text-sm font-medium text-white bg-sky-300 rounded-lg border border-sky-300 hover:bg-sky-400 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                        >
            ...
            <section className="text-gray-600 body-font">
                <div className="container px-10 py-5 ml-2 mx-auto ">
